...
for /f %%i in ('netstat -na ^| findstr "58801" ^| findstr "LISTENING"') do set _vault_up=1
...
%convert% -i http://%_hostname%:58801/v1/swagger.json -d /opt/vault -c /opt/config.properties
...
set args=--input-file=http://%_hostname%:58801/v1/swagger.json
docker run --rm --mount type=bind,source=%cd%,target=/opt -w /opt azuresdk/autorest:latest %args%
...
curl -o vault.json http://%_hostname%:58801/v1/swagger.json
popd
goto :eof
